...
* Check the `ConnectionStrings` in `appsettings.json` files under the `laundry_SaaS.HttpApi.Host` and `laundry_SaaS.DbMigrator` projects and change it if you need.
...
```bash
dotnet dev-certs https -v -ep openiddict.pfx -p 09ad97b7-4c62-418f-88ed-2031ef34625a
```

> `09ad97b7-4c62-418f-88ed-2031ef34625a` is the password of the certificate, you can change it to any password you want.
...
Run `laundry_SaaS.DbMigrator` to create the initial database. This should be done in the first run. It is also needed if a new database migration is added to the solution later.

### Solution structure

This is a layered monolith application that consists of the following applications:

* `laundry_SaaS.DbMigrator`: A console application which applies the migrations and also seeds the initial data. It is useful on development as well as on production environment.
* `laundry_SaaS.HttpApi.Host`: ASP.NET Core API application that is used to expose the APIs to the clients.
