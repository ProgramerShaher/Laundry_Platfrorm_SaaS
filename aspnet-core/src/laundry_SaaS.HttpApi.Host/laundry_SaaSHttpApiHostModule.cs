...
public class laundry_SaaSHttpApiHostModule : AbpModule
{
    ...
    private void ConfigureVirtualFileSystem(ServiceConfigurationContext context)
    {
        ...
        if (hostingEnvironment.IsDevelopment())
        {
            ...
            {
                options.FileSets.ReplaceEmbeddedByPhysical<laundry_SaaSDomainSharedModule>(
                    Path.Combine(hostingEnvironment.ContentRootPath,
                        $"..{Path.DirectorySeparatorChar}laundry_SaaS.Domain.Shared"));
                options.FileSets.ReplaceEmbeddedByPhysical<laundry_SaaSDomainModule>(
                    Path.Combine(hostingEnvironment.ContentRootPath,
                        $"..{Path.DirectorySeparatorChar}laundry_SaaS.Domain"));
                options.FileSets.ReplaceEmbeddedByPhysical<laundry_SaaSApplicationContractsModule>(
                    Path.Combine(hostingEnvironment.ContentRootPath,
                        $"..{Path.DirectorySeparatorChar}laundry_SaaS.Application.Contracts"));
                options.FileSets.ReplaceEmbeddedByPhysical<laundry_SaaSApplicationModule>(
                    Path.Combine(hostingEnvironment.ContentRootPath,
    ...
    private void ConfigureConventionalControllers()
    {
        ...
        {
            options.ConventionalControllers.Create(typeof(laundry_SaaSApplicationModule).Assembly);
        });
    }
    ...
    private static void ConfigureSwaggerServices(ServiceConfigurationContext context, IConfiguration configuration)
    {
        context.Services.AddAbpSwaggerGenWithOAuth(
            ...
            {
                    {"laundry_SaaS", "laundry_SaaS API"}
            },
            options =>
            {
                options.SwaggerDoc("v1", new OpenApiInfo { Title = "laundry_SaaS API", Version = "v1" });
    ...
    public override void OnApplicationInitialization(ApplicationInitializationContext context)
    {
        ...
        {
            c.SwaggerEndpoint("/swagger/v1/swagger.json", "laundry_SaaS API");
